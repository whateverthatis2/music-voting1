...
SAM1_TABS = [
    ("home_sam1",     "/sam1",          "Чек-лист Сам.1"),
]

SAM2_TABS = [
    ("home_sam2",     "/sam2",          "Експерименти Сам.2"),
]
...
_HEADERS = {
    ...
    8: ("Лабораторна робота №8 — максимальне прискорення системи (закони Амдала 2 і 3)",
        "Інтелектуальна обробка даних в розподілених інформаційних середовищах · КН-41"),
    101: ("Самостійна робота №1 — програмний модуль розподіленого ранжування",
        "Інтелектуальна обробка даних в розподілених інформаційних середовищах · КН-41"),
    102: ("Самостійна робота №2 — експериментальне дослідження ранжування",
        "Інтелектуальна обробка даних в розподілених інформаційних середовищах · КН-41"),
}
...
def page(title: str, body: str, active: str = "",
    ...
    if lab_num == 0:
        title_suffix = "ІОД РІС"
    elif lab_num >= 101:  # 101 = Сам.1, 102 = Сам.2 (умовна нумерація)
        title_suffix = f"Самостійна №{lab_num - 100}"
    else:
        title_suffix = f"Лабораторна №{lab_num}"
    nav_html = "".join(
